...
| **STS** `AssumeRole` | 600 per sec, shared across all STS calls (adjustable via support) | **hub account**, per region — cross-account assumes only consume the *caller's* quota, never the target's | Low. Exactly **1 `AssumeRole` per (account, region)** — so 700 accounts × 5 regions = 3,500 calls per run, all against the hub's budget. |
...
By default all `AssumeRole` calls go to the STS endpoint of the region the
hub pod runs in — one region's 600/s budget for every account. Setting:
...
load spreads across as many regional budgets as you scan. Free, no code
changes, and AWS recommends regional STS endpoints generally (lower latency,
no dependency on a single-region global endpoint).
